...
import { MathisDialogComponent } from "../shared-utilities/mathis-dialog/mathis-dialog.component";
import { Overlay } from "@angular/cdk/overlay";
import { Router } from "@angular/router";
import { UserProfile } from "../core/api/v1/model/userProfile";
...
export class AppUtilityService extends AppDictionaryService {
  ...
  serviceWrapper(
    ...
    this.http
      .request(HTTP_method, API_URL, request_data)
      .pipe(
        finalize(() => {
          if (!!!skip_loading_animation) {
  ...
  displayAlertDialog(options?: any) {
    let global_options = {
      autoFocus: false,
      panelClass: "mathis-dialog-container",
      scrollStrategy: this.overlay.scrollStrategies.noop(),
    };
    let dialog_config = { ...global_options, ...options };
    let dialog_ref = this.dialog.open(MathisDialogComponent, dialog_config);
    return dialog_ref;
  }
